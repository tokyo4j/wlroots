...
static const struct wlr_input_filter_focus *filter_focus(
...
static void dismiss_grab(struct wlr_xdg_popup_grab_input_filter_layer *layer) {
	struct wlr_xdg_popup *popup = layer->popup;
	wlr_xdg_popup_grab_input_filter_layer_destroy(layer);
...
static uint32_t keyboard_focus(struct wlr_input_filter_keyboard *keyboard,
	...
	struct wlr_input_filter_keyboard_focus_event relayed = *event;
	relayed.focus = &layer->keyboard_focus;
	return wlr_input_filter_keyboard_notify_focus(keyboard, &relayed);
}

static const struct wlr_input_filter_keyboard_impl keyboard_impl = {
	.name = "wlr_xdg_popup_grab_input_filter_layer-keyboard",
...
static uint32_t pointer_position(struct wlr_input_filter_pointer *pointer,
	...
	struct wlr_xdg_popup_grab_input_filter_layer *layer = wl_container_of(pointer, layer, pointer);

	struct wlr_input_filter_pointer_position_event relayed = *event;
	relayed.focus = event->explicit_focus ? event->focus : filter_focus(layer, event->focus);
	relayed.explicit_focus = true;
	return wlr_input_filter_pointer_notify_position(pointer, &relayed);
}
...
static const struct wlr_input_filter_pointer_impl pointer_impl = {
	.name = "wlr_xdg_popup_grab_input_filter_layer-pointer",
...
static void touch_position(struct wlr_input_filter_touch *touch,
	...
	struct wlr_xdg_popup_grab_input_filter_layer *layer = wl_container_of(touch, layer, touch);

	struct wlr_input_filter_touch_position_event relayed = *event;
	relayed.focus = filter_focus(layer, event->focus);
	wlr_input_filter_touch_notify_position(touch, &relayed);
}
...
static uint32_t touch_down(struct wlr_input_filter_touch *touch,
	...
	relayed.focus = filter_focus(layer, event->focus);
	uint32_t serial = wlr_input_filter_touch_notify_down(touch, &relayed);
...
static const struct wlr_input_filter_touch_impl touch_impl = {
	.name = "wlr_xdg_popup_grab_input_filter_layer-touch",
	.position = touch_position,
	.down = touch_down,
};

static void set_topmost_popup(struct wlr_xdg_popup_grab_input_filter_layer *layer,
	...
	wlr_input_filter_focus_set_surface(&layer->keyboard_focus, popup->base->surface);
	wlr_input_filter_keyboard_notify_focus(&layer->keyboard,
...
static const struct wlr_addon_interface router_addon_impl = {
	.name = "wlr_xdg_popup_grab_input_filter_layer",
...
struct wlr_xdg_popup_grab_input_filter_layer *wlr_xdg_popup_grab_input_filter_layer_get_or_create(
	...
	wlr_input_filter_focus_init(&layer->keyboard_focus);
...
void wlr_xdg_popup_grab_input_filter_layer_destroy(
	...
	wlr_input_filter_focus_finish(&layer->keyboard_focus);
