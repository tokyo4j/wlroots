...
static void update_focus(struct wlr_input_filter_focus_layer *layer, double x, double y) {
	wlr_input_filter_at(layer->router, x, y, &layer->focus, NULL, NULL);
}
...
static uint32_t pointer_position(struct wlr_input_filter_pointer *pointer,
	...
	struct wlr_input_filter_focus_layer *layer = wl_container_of(pointer, layer, pointer);
...
static const struct wlr_input_filter_pointer_impl pointer_impl = {
	.base = {
		.name = "wlr_input_filter_focus_layer-pointer",
	},
...
static void touch_position(struct wlr_input_filter_touch *touch,
	...
	struct wlr_input_filter_focus_layer *layer = wl_container_of(touch, layer, touch);
	update_focus(layer, event->x, event->y);
...
static uint32_t touch_down(struct wlr_input_filter_touch *touch,
	...
	relayed.focus = &layer->focus;
	return wlr_input_filter_touch_notify_down(touch, &relayed);
}

static const struct wlr_input_filter_touch_impl touch_impl = {
	.base = {
		.name = "wlr_input_filter_focus_layer-touch",
	},
...
static void handle_router_destroy(struct wl_listener *listener, void *data) {
	struct wlr_input_filter_focus_layer *layer = wl_container_of(listener, layer, router_destroy);
	wlr_input_filter_focus_layer_destroy(layer);
}
...
struct wlr_input_filter_focus_layer *wlr_input_filter_focus_layer_create(
	...
	struct wlr_input_filter_focus_layer *layer = calloc(1, sizeof(*layer));
	if (layer == NULL) {
	...
	wlr_input_filter_focus_init(&layer->focus);
...
void wlr_input_filter_focus_layer_destroy(struct wlr_input_filter_focus_layer *layer) {
	...
	wlr_input_filter_focus_finish(&layer->focus);
