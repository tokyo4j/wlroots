...
static uint32_t pointer_position(struct wlr_input_filter_pointer *pointer,
	...
	struct wlr_input_filter_implicit_grab_layer *layer = wl_container_of(pointer, layer, pointer);
	...
	if (!event->explicit_focus && layer->pointer_grabbed) {
		...
		wlr_input_filter_focus_copy(&layer->pointer_focus, event->focus);
	}
	return wlr_input_filter_pointer_notify_position(pointer, event);
}
...
static uint32_t pointer_button(struct wlr_input_filter_pointer *pointer,
	...
	if (event->state == WL_POINTER_BUTTON_STATE_PRESSED) {
		...
		if (pointer->n_buttons == 0) {
			...
			wlr_input_filter_pointer_refresh_position(pointer);
		}
	}
...
static const struct wlr_input_filter_pointer_impl pointer_impl = {
	.base = {
		.name = "wlr_input_filter_implicit_grab_layer-pointer",
	},
	.position = pointer_position,
	.button = pointer_button,
};

static void touch_position(struct wlr_input_filter_touch *pointer,
	...
	wlr_input_filter_touch_notify_position(pointer, &relayed);
}

static uint32_t touch_down(struct wlr_input_filter_touch *pointer,
		const struct wlr_input_filter_touch_down_event *event) {
	struct wlr_input_filter_implicit_grab_layer *layer =
		wl_container_of(pointer, layer, touch);

	uint32_t serial = wlr_input_filter_touch_notify_down(pointer, event);

	struct wlr_input_filter_implicit_grab_layer_touch_point *point =
...
static const struct wlr_input_filter_touch_impl touch_impl = {
	.base = {
		.name = "wlr_input_filter_implicit_grab_layer-touch",
	},
...
static void handle_router_destroy(struct wl_listener *listener, void *data) {
	...
	wlr_input_filter_implicit_grab_layer_destroy(layer);
}
...
bool wlr_input_filter_implicit_grab_layer_validate_pointer_serial(
	...
	if (origin != NULL && origin != wlr_input_filter_focus_get_surface(&layer->pointer_focus)) {
		return false;
	}
...
bool wlr_input_filter_implicit_grab_layer_validate_touch_serial(
	...
	for (size_t i = 0; i < layer->touch.n_points; i++) {
		struct wlr_input_filter_implicit_grab_layer_touch_point *point = &layer->touch_points[i];
		if (layer->touch_points[i].serial != serial) {
			continue;
		}
		if (origin != NULL && origin != wlr_input_filter_focus_get_surface(&point->focus)) {
			continue;
		}
...
struct wlr_input_filter_implicit_grab_layer *wlr_input_filter_implicit_grab_layer_create(
	...
	struct wlr_input_filter_implicit_grab_layer *layer = calloc(1, sizeof(*layer));
	if (layer == NULL) {
	...
	wlr_input_filter_focus_init(&layer->pointer_focus);

	wlr_input_filter_touch_init(&layer->touch, router, &touch_impl);
	for (size_t i = 0; i < WLR_INPUT_FILTER_MAX_TOUCH_POINTS; i++) {
		wlr_input_filter_focus_init(&layer->touch_points[i].focus);
	}
...
void wlr_input_filter_implicit_grab_layer_destroy(
	...
	wlr_input_filter_focus_finish(&layer->pointer_focus);

	wlr_input_filter_touch_finish(&layer->touch);
	for (size_t i = 0; i < WLR_INPUT_FILTER_MAX_TOUCH_POINTS; i++) {
		wlr_input_filter_focus_finish(&layer->touch_points[i].focus);
	}
