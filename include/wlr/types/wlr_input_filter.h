...
#ifndef WLR_TYPES_WLR_INPUT_FILTER_H
#define WLR_TYPES_WLR_INPUT_FILTER_H
...
#define WLR_INPUT_FILTER_MAX_POINTER_BUTTONS 32
#define WLR_INPUT_FILTER_MAX_TOUCH_POINTS 16

enum wlr_input_filter_focus_type {
	WLR_INPUT_FILTER_FOCUS_NONE,
	WLR_INPUT_FILTER_FOCUS_SURFACE,
	WLR_INPUT_FILTER_FOCUS_USER,
};
 ...
 * empty focus.
 */
struct wlr_input_filter_focus {
	enum wlr_input_filter_focus_type type;
	union {
...
struct wlr_input_filter_manager;

struct wlr_input_filter_interface {
	const char *name;

	void (*at)(struct wlr_input_filter_manager *manager, double x, double y,
		struct wlr_input_filter_focus *focus, double *local_x, double *local_y);
	bool (*get_surface_position)(struct wlr_input_filter_manager *manager,
		struct wlr_surface *surface, double *x, double *y);
};

struct wlr_input_filter_keyboard;

/**
 * Event notifying of a new keyboard focus. It is not guaranteed that the focus
 * has actually changed.
 */
struct wlr_input_filter_keyboard_focus_event {
 ...
 */
struct wlr_input_filter_keyboard_key_event {
	uint32_t time_msec;
	uint32_t key;
 ...
 */
struct wlr_input_filter_keyboard_modifiers_event {
	struct {
		char unused;
	} WLR_PRIVATE;
};

struct wlr_input_filter_keyboard_impl {
	const char *name;

	uint32_t (*focus)(struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_focus_event *event);
	void (*device)(struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_device_event *event);
	uint32_t (*key)(struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_key_event *event);
	void (*modifiers)(struct wlr_input_filter_keyboard *keyboard,
...
struct wlr_input_filter_keyboard {
	...
	struct wlr_input_filter_focus focus;
	struct wlr_keyboard *device;
 ...
 */
struct wlr_input_filter_pointer_button_event {
	uint32_t time_msec;
...
struct wlr_input_filter_pointer_axis_event {
	uint32_t time_msec;
...
struct wlr_input_filter_pointer_frame_event {
	struct {
		char unused;
	} WLR_PRIVATE;
};

struct wlr_input_filter_pointer;

struct wlr_input_filter_pointer_impl {
	const char *name; // for debugging

	uint32_t (*position)(struct wlr_input_filter_pointer *pointer,
		const struct wlr_input_filter_pointer_position_event *event);
	uint32_t (*button)(struct wlr_input_filter_pointer *pointer,
		const struct wlr_input_filter_pointer_button_event *event);
	void (*axis)(struct wlr_input_filter_pointer *pointer,
		const struct wlr_input_filter_pointer_axis_event *event);
	void (*frame)(struct wlr_input_filter_pointer *pointer,
		const struct wlr_input_filter_pointer_frame_event *event);
};

struct wlr_input_filter_pointer_button {
	uint32_t button;
...
struct wlr_input_filter_pointer {
	...
	struct wlr_input_filter_pointer_button buttons[WLR_INPUT_FILTER_MAX_POINTER_BUTTONS];
	uint32_t n_buttons;
};
 ...
 * point ID.
 */
struct wlr_input_filter_touch_position_event {
	uint32_t time_msec;
	int32_t id;
 ...
 */
struct wlr_input_filter_touch_down_event {
	uint32_t time_msec;
	int32_t id;
 ...
 */
struct wlr_input_filter_touch_up_event {
	uint32_t time_msec;
	int32_t id;
...
struct wlr_input_filter_touch_frame_event {
	struct {
		char unused;
	} WLR_PRIVATE;
};

struct wlr_input_filter_touch;

struct wlr_input_filter_touch_impl {
	const char *name;

	void (*position)(struct wlr_input_filter_touch *touch,
		const struct wlr_input_filter_touch_position_event *event);
	uint32_t (*down)(struct wlr_input_filter_touch *touch,
		const struct wlr_input_filter_touch_down_event *event);
	uint32_t (*up)(struct wlr_input_filter_touch *touch,
		const struct wlr_input_filter_touch_up_event *event);
	void (*cancel)(struct wlr_input_filter_touch *touch,
		const struct wlr_input_filter_touch_cancel_event *event);
	void (*frame)(struct wlr_input_filter_touch *touch,
		const struct wlr_input_filter_touch_frame_event *event);
};

struct wlr_input_filter_touch_point {
	int32_t id;
...
struct wlr_input_filter_touch {
	...
	struct wlr_input_filter_touch_point points[WLR_INPUT_FILTER_MAX_TOUCH_POINTS];
	size_t n_points;
};
 ...
 */
struct wlr_input_filter_manager {
	struct wl_list keyboard_filters;
	struct wl_list pointer_filters;
	struct wl_list touch_filters;
 ...
 */
struct wlr_input_filter_focus_layer {
...
struct wlr_input_filter_implicit_grab_layer_touch_point {
	struct {
		uint32_t serial;
		struct wlr_input_filter_focus focus;
	} WLR_PRIVATE;
 ...
 * stays the same.
 */
struct wlr_input_filter_implicit_grab_layer {
	struct wlr_input_filter *filter;
	...
	struct {
		struct wlr_input_filter_pointer pointer;
		struct wlr_input_filter_focus pointer_focus;
...
bool wlr_input_filter_get_surface_position(struct wlr_input_filter_manager *manager,
		struct wlr_surface *surface, double *x, double *y);

uint32_t wlr_input_filter_keyboard_notify_focus(
		struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_focus_event *event);

void wlr_input_filter_keyboard_notify_device(
		struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_device_event *event);

uint32_t wlr_input_filter_keyboard_notify_key(
		struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_key_event *event);

void wlr_input_filter_keyboard_notify_modifiers(
		struct wlr_input_filter_keyboard *keyboard,
		const struct wlr_input_filter_keyboard_modifiers_event *event);
...
uint32_t wlr_input_filter_pointer_notify_position(
		struct wlr_input_filter_pointer *pointer,
		const struct wlr_input_filter_pointer_position_event *event);

uint32_t wlr_input_filter_pointer_notify_button(
		struct wlr_input_filter_pointer *pointer,
		const struct wlr_input_filter_pointer_button_event *event);
...
bool wlr_input_filter_focus_layer_register(int32_t priority);

struct wlr_input_filter_focus_layer *wlr_input_filter_focus_layer_create(
		struct wlr_input_filter_manager *manager);
void wlr_input_filter_focus_layer_destroy(struct wlr_input_filter_focus_layer *layer);

bool wlr_input_filter_implicit_grab_layer_register(int32_t priority);

struct wlr_input_filter_implicit_grab_layer *wlr_input_filter_implicit_grab_layer_create(
		struct wlr_input_filter *filter);
void wlr_input_filter_implicit_grab_layer_destroy(
		struct wlr_input_filter_implicit_grab_layer *layer);

bool wlr_input_filter_implicit_grab_layer_validate_pointer_serial(
		struct wlr_input_filter_implicit_grab_layer *layer, struct wlr_surface *origin,
		uint32_t serial, uint32_t *button);

bool wlr_input_filter_implicit_grab_layer_validate_touch_serial(
		struct wlr_input_filter_implicit_grab_layer *layer, struct wlr_surface *origin,
